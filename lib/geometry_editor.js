...
class GeomBase {
    ...
    _removeDependantEntity(shape) {
        ...
        if (!_shape) { throw new Error("expecting shape to be found on _dependencies " + shape._id); }
...
class ShapeConnection {
    constructor(parent) {
        ...
        assert(!parent || parent instanceof GeomBase, "expecting a parent");

        this._parent = parent;
        this._linkedShape = null;

        if (this._parent) {
            this._parent._shapeConnectors = this._parent._shapeConnectors || [];
            this._parent._shapeConnectors.push(this);
        }
    }

    set(shape) {
        assert(this._parent instanceof GeomBase);
        assert(!shape || shape instanceof GeomBase);
        this.__removeDependantEntity();
        this._linkedShape = shape;
        this.__addDependantEntity();
    }

    get() {
        return this._linkedShape;
    }

    __addDependantEntity() {
        if (this._parent._id && this._linkedShape) {
            this._linkedShape._addDependantEntity(this._parent);
        }
    }

    __removeDependantEntity() {
        if (this._parent._id && this._linkedShape) {
            this._linkedShape._removeDependantEntity(this._parent);
        }
    }

    _replaceLink(oldShape, newShape) {
        assert(oldShape instanceof GeomBase);
        assert(newShape instanceof GeomBase);
        if (this.get() === oldShape) {
            this.set(newShape);
        }
    }
...
class GeomOperation extends GeomBase {
    ...
    constructor(name, arg1, arg2) {
        ...
        this.leftArg = new ShapeConnection(this);
        this.rightArg = new ShapeConnection(this);
...
function t(arg) {
    assert(arg instanceof ShapeConnection, "expecting a ShapeConnection");
...
class GeomOperationFuse extends GeomOperation {
    ...
    clone() {
        const clone = new GeomOperationFuse(this.name, this.leftArg.get(), this.rightArg.get());
        return clone;
    }
...
class GeomOperationCommon extends GeomOperation {
    ...
    clone() {
        const clone = GeomOperationCommon(this.name, this.leftArg.get(), this.rightArg.get());
        return clone;
    }
...
class GeomTransfo extends GeomBase {
    ...
    constructor(name) {
        super(name);
        this.geometry = new ShapeConnection(this);
    }

    setGeometry(geo) {
        assert(geo instanceof GeomBase);
        assert(this.geometry instanceof ShapeConnection);
...
class GeometryEditor {
    ...
    convertToScript() {
        ...
        const lines = this.elements.map(convertItemToScript);
        return lines.join("\n");
    }
...
serialijse.declarePersistable(ShapeConnection);
